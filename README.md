...
Application desktop autonome pour animer un loto français (bingo 1 à 90), maintenant développée entièrement en **Java**.
>>>>>>> theirs
...
=======
- Boutons d'annonce : **Quine simple**, **Quine double**, **Carton plein**.
- Interface moderne (fond dégradé, hover boutons, effets visuels sur tirage).

## Prérequis

- JDK 17+ installé (avec `javac`, `java`, `jar`, `jpackage`).
...
## Générer un `.exe` autonome (Windows)

### Option 1 (scripts)

- `build_exe_java.bat` (cmd)
- `build_exe_java.ps1` (PowerShell)
>>>>>>> theirs
...
Le `.exe` final est généré dans le dossier `LotoFR/` (créé par `jpackage`).

## Notes

- Cette version ne dépend d'aucun serveur.
